...
\end{align*}
\subsubsection{}
Avec les conditions initiales la méthode de Gauss-Newton 
\begin{figure}[H]
	\begin{center}
...
\end{figure}
\subsubsection{}
D'après la figure \ref{fig:I23} on voit la divergence de la méthode de Gauss-Newton avec des conditions initiales proches de la solution trouvé dans l'item précédent. De cette manière on voit la non-convergence locale de cette approximation de la
\begin{figure}[H]
	\begin{center}
...
\begin{align*}
u_i^n = \left ( -\frac{b \Delta t}{\Delta x^2} \right) u_{i+1}^{n+1} +\left ( 1 + 2 \frac{b \Delta t}{\Delta x^2} \right )  u_{i}^{n+1}  + \left ( -\frac{b \Delta t}{\Delta x^2} \right) u_{i-1}^{n+1} + u_i^{n+1}(u_i^n)^3 \text{.}
\end{align*}
Ainsi, on peu écrire $U^n=(A+ \Delta t \text{diag}((u_{-i_{\text{max}}}^n), \dots, (u_{i_{\text{max}}}^n)))U^{n+1}$
, où $U^n= \begin{pmatrix} u_{-i_\text{max}}^n \\ \vdots \\ u_{i_\text{max}}^n \end{pmatrix}$, $\delta=\frac{b \Delta t}{\Delta x^2}$ et $A_{ij}= \begin{cases}
1 + 2 \delta &\text{, si } i=j \\
